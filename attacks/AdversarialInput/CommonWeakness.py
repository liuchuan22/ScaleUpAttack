...
class BIM_CosineSimilarityEncourager(MI_CosineSimilarityEncourager):
    def __init__(self, *args, **kwargs):
        super(BIM_CosineSimilarityEncourager, self).__init__(*args, **kwargs)
    ...
    def attack(self, x, y):
        ...
        for _ in range(self.total_step):
            ...
            for model in self.models:
                ...
                # update
                if self.targerted_attack:
                    update = -grad / torch.norm(grad.reshape(N, -1), p=2, dim=1).view(N, 1, 1, 1)
                    x += self.step_size * update
                else:
                    update = grad / torch.norm(grad.reshape(N, -1), p=2, dim=1).view(N, 1, 1, 1)
                    x += self.step_size * update
                    # x += self.step_size * grad / torch.norm(grad.reshape(N, -1), p=2, dim=1).view(N, 1, 1, 1)
...
class MI_CommonWeakness(AdversarialInputAttacker):
    ...
    def attack(self, x, y):
        ...
        for _ in range(self.total_step):
            ...
            for model in self.models:
                ...
                # loss = self.criterion(model(aug_x.to(model.device)))
                loss.backward()
                grad = x.grad
                self.grad_record.append(grad)
                x.requires_grad = False
    ...
    def end_attack(self, now: torch.tensor):
        ...
        now.add_(self.step_size * self.outer_momentum.sign())
...
class BIM_CommonWeakness(MI_CommonWeakness):
    def __init__(self, *args, **kwargs):
        ...
        for _ in range(self.total_step):
            ...
            for model in self.models:
                ...
                if self.TI:
                    grad = self.ti(grad)
                if self.targerted_attack:
                    inner_update = -grad / torch.norm(grad.reshape(N, -1), p=2, dim=1).view(N, 1, 1, 1)
                    x += self.inner_step_size * inner_update
                else:
                    inner_update = grad / torch.norm(grad.reshape(N, -1), p=2, dim=1).view(N, 1, 1, 1)
                    x += self.inner_step_size * inner_update
                x = clamp(x)
    ...
    def end_attack(self, now: torch.tensor):
        ...
        """
        fake_grad = now - self.original  # x_n-x
        outer_update = fake_grad / torch.norm(fake_grad, p=1)
        now.mul_(0)
        now.add_(self.original)
        now.add_(self.step_size * outer_update.sign())
        # patch.add_(ksi * fake_grad)  Without momentum
        now = clamp(now)
        # grad_similarity = cosine_similarity(self.grad_record)
        del self.grad_record
